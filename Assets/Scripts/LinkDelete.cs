...
public class LinkDelete : MonoBehaviour
{
    ...
    public void UpdateItems()
    {
        ...
        using (var connection = new SqliteConnection(dbName))
        {
            ...
            {
                command.CommandText = "select op.ID as opid, o.Obozn as oob, o.Naim as ona, p.Obozn as pob, p.Naim as pna, op.Kolvo as kolvo from OPReference op inner join Objects o on(o.ID = op.ObjectID) inner join Products p on(p.ID = op.ProductID) ";
                if (objectSearch.text != "")
                {
                    command.CommandText = command.CommandText + "where o.Obozn like '%" + objectSearch.text + "%' or p.Obozn like '%" + objectSearch.text + "%';";
                }
                command.CommandText = command.CommandText + "order by o.ID;";
    ...
    void InitializeItemView(GameObject viewGameObject, ItemModel model)
    {
        ...
        view.id = model.id;
        view.titleText.text = model.title + " " + model.title2 + " - " + model.title3 + " " + model.title4 + "\nв количестве " + model.kolvo;
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                using (IDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        results[i].id = Convert.ToInt32(reader["opid"]);
                        results[i].title = (reader["oob"].ToString());
                        results[i].title2 = (reader["ona"].ToString());
                        results[i].title3 = (reader["pob"].ToString());
                        results[i].title4 = (reader["pna"].ToString());
                        results[i].kolvo = (reader["kolvo"].ToString());
                        i++;
                    }
                    reader.Close();
                }
